 ...
 * @description:
 **/
public class GcsGovMoMac {
    private static final Log log = LogFactory.getLog(GcsGovMoMac.class);
    ...
    static {
        ...
        header.put("wicket-ajax-baseurl", "list/zh-hant/news/?2");
        header.put("x-requested-with", "XMLHttpRequest");
        header.put("referer", "https://www.gcs.gov.mo/news/list/zh-hans/news/?2");
    }
        ...
        while (inThreeMonth) {
            ...
            try {
                if (StringUtils.isNotBlank(key)) {
                    String url = "https://www.gcs.gov.mo/news/list/zh-hant/news" + key.substring(1);
                    search = Jsoup.parse(RequestUtil.proxyGet(url, proxyHost, proxyPort, header));
                    log.info(url);
                } else {
                    // 获取首次页面
                    search = Jsoup.parse(RequestUtil.proxyGet("https://www.gcs.gov.mo/news/list/zh-hant/news/?2", proxyHost, proxyPort, Maps.newHashMap()));
                    // 获取下一页的key
                    key = JsoupUtil.getFirstElementByClass(search, "infiniteNext nextItems", "href");
                    log.info("https://www.gcs.gov.mo/news/list/zh-hant/news/?2");
                }
            } catch (IOException e) {
                continue;
            }
